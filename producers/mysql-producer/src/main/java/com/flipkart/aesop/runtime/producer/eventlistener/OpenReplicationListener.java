...
public class OpenReplicationListener implements BinlogEventListener
{
	...
	{
		LOGGER.warn(">>Aesop open replicator exception occured >>>>>>>>>>>>>>>>");
		//Code to send email when the mysql server crashes
		try 
		{
            		Process proc = Runtime.getRuntime().exec("/srv/aesop_zopper/utilities/custom_utils/mail.sh /"); //Whatever you want to execute
