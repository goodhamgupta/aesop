...
public class DefaultBinLogEventMapper<T extends GenericRecord> implements BinLogEventMapper<T>
{
	...
	public T mapBinLogEvent(BinlogEventV4Header header, Row row, DbusOpcode databusCode, Schema schema)
	{
		...
		try
		{
			...
			int cnt = 0;
			LOGGER.info("########### Number of avro field in list is "+Integer.toString( orderedFields.size()));
			LOGGER.info("########### Number of columns in row is "+Integer.toString( columns.size()));
			for (Schema.Field field : orderedFields)
			{
				if(columns.size() <= cnt)
				{
					LOGGER.info("############ No further columns found so breaking out of the loop.");
					break;
				}
